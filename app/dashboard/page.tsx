...
<div className="relative min-h-[calc(100vh-8rem)] overflow-hidden bg-[#050606] text-white" style={teamStyles} > {hqConfig ? ( <section className="relative isolate min-h-[520px] w-full overflow-hidden border-y border-white/10 bg-black sm:min-h-[580px] lg:aspect-[16/9] lg:min-h-0">
<Image src={hqConfig.background} alt={`${team.city} ${team.name} executive headquarters`} fill priority sizes="100vw" className="object-cover object-center brightness-[1.03] contrast-[1.02] saturate-[1.04]" /> {/* Gentle cinematic treatment without hiding the office. */} <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.03)_0%,rgba(0,0,0,0.02)_50%,rgba(0,0,0,0.42)_100%)]" />
<div className="pointer-events-none absolute inset-y-0 left-0 w-[44%] bg-[linear-gradient(90deg,rgba(2,4,7,0.34)_0%,rgba(2,4,7,0.12)_60%,transparent_100%)]" /> {/* Compact interface panel. The office remains the hero. */} <div className="absolute bottom-6 left-6 z-10 w-[calc(100%-3rem)] max-w-[430px] rounded-2xl border border-white/15 bg-black/42 p-5 shadow-[0_18px_50px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:bottom-8 sm:left-8 sm:p-6 lg:bottom-10 lg:left-10">
<div className="flex items-center gap-4">
<div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-white/15 bg-black/35">
<Image src={teamLogo} alt={`${team.city} ${team.name}`} fill unoptimized className="object-contain p-2.5" />
</div>
<div className="min-w-0">
<p className="text-[10px] font-black uppercase tracking-[0.28em]" style={{ color: team.secondary }} > NEW ERA FRANCHISE HQ </p>
<h1 className="mt-1 text-3xl font-black tracking-[-0.045em] sm:text-4xl"> {team.city} {team.name} </h1>
</div>
</div>
<p className="mt-4 text-sm leading-6 text-zinc-200 sm:text-base"> Welcome back,{" "} <span className="font-black text-white"> {member?.display_name ?? user.displayName} </span> . Your franchise operations are ready. </p>
<div className="mt-5 flex flex-wrap gap-3">
<Link href={teamPath} className="rounded-xl px-4 py-2.5 text-sm font-black text-white transition hover:-translate-y-0.5" style={{ background: `linear-gradient(135deg, ${team.primary}, ${team.secondary})`, }} > Enter Team HQ </Link>
<Link href={`${teamPath}/roster`} className="rounded-xl border border-white/15 bg-black/25 px-4 py-2.5 text-sm font-black text-white transition hover:-translate-y-0.5 hover:bg-white/[0.08]" > Manage Roster </Link>
</div>
</div>
